...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01_SnowflakeCore.ipynb.

# %% auto 0
__all__ = ['model_name', 'UNSAFE_QUERIES', 'FIELD_TO_FILTER', 'CHACHE_DIR', 'connect_to_snowflake', 'SnowflakeAgent', 'TableAttr',
           'ParentSchema', 'DBMetadata']

# %% ../nbs/01_SnowflakeCore.ipynb 5
...
class SnowflakeAgent(DatabaseAgent):
    ...
    def execute_query(
        ...
        """
        Execute a safe SQL SELECT query against Snowflake and return structured results.
        SAFETY CONSTRAINTS:
        - Only SELECT queries allowed (no DROP, DELETE, UPDATE, INSERT, ALTER, TRUNCATE)
        - Single statement only (no semicolon-separated multiple queries)
        - Row limiting enforced unless fetch_all=True
        Args:
            query: SQL SELECT statement to execute
            max_rows: Maximum rows to return (default: 10). Ignored if fetch_all=True
            fetch_all: If True, return all matching rows. Use cautiously for large datasets
        Returns:
            QueryResult with fields: success, data, error, row_count, execution_time
        Examples:
    ...
    def extract_metadata(self, **kwargs):
        return super().extract_metadata(**kwargs)

# %% ../nbs/01_SnowflakeCore.ipynb 33
...
# %% ../nbs/01_SnowflakeCore.ipynb 83
CHACHE_DIR = Path('../cache')
CHACHE_DIR.mkdir(exist_ok=True)

# %% ../nbs/01_SnowflakeCore.ipynb 92
@patch(as_prop=True)
def fn(self:DBMetadata):
    return CHACHE_DIR / f"{self.__hash__()}.json"

# %% ../nbs/01_SnowflakeCore.ipynb 95
@patch(as_prop=True)
def metadata2json_str(self:DBMetadata):
    return self.metadata.model_dump_json(indent=2)   # or .dict() + json.dumps(...)

# %% ../nbs/01_SnowflakeCore.ipynb 97
@patch
def load_metadata(self:DBMetadata, json_str):
    metadata_dict = json.loads(json_str)
    self.metadata = ParentSchema(**metadata_dict)

# %% ../nbs/01_SnowflakeCore.ipynb 100
@patch
def metadata2disk(self:DBMetadata):
    self.fn.write_text(self.metadata2json_str, "utf-8")

@patch
def disk2metadata(self:DBMetadata):
    data = self.fn.read_text("utf-8")
    self.load_metadata(data)

# %% ../nbs/01_SnowflakeCore.ipynb 106
@patch
def __call__(self:DBMetadata, use_cache:bool = True):
